...
HEADER_COMMENT = "/* This file is automatically generated by Bazel. Do not edit! */"
...
def vpx_config_header(name, arch, out, features = {}, windows = False):
    ...
    lines = [HEADER_COMMENT, "#ifndef VPX_CONFIG_H", "#define VPX_CONFIG_H", "#define RESTRICT    ", "#define INLINE      inline"]
...
def vpx_config_asm(name, arch, out, features = {}, windows = False):
    ...
    if arch in ["x86", "x86_64"]:
        ...
        lines = ["@ This file was created from a .asm file", "@  using the ads2gas.py script.", ".syntax unified"]
...
def vpx_config_rtcd(name, arch, out, features = {}, windows = False):
    ...
    Args:
        name: Rule name.
        arch: "x86", "x86_64", "arm", or "arm64".
        out: Output file path for the config.
        features: Dict of CONFIG_* overrides.
        windows: Whether to apply Windows-specific overrides.
    """
    defines = _build_defines(arch, features, windows = windows)
...
def vpx_config_c(name, arch, out, features = {}, windows = False):
    """Generates a vpx_config.c file containing the build configuration string.

    Args:
        name: Rule name.
        arch: "x86", "x86_64", "arm", or "arm64".
        out: Output file path for vpx_config.c.
        features: Dict of CONFIG_* overrides.
        windows: Whether to apply Windows-specific overrides.
    """
    defines = _build_defines(arch, features, windows = windows)
    ...
    for k, v in sorted(defines.items()):
        if k.startswith("CONFIG_") or k.startswith("HAVE_"):
            if str(v) == "1":
    ...
    content = [
        ...
        'static const char* const cfg = "%s";' % config_str,
