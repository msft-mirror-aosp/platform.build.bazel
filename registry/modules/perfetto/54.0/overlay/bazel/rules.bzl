...
"""This file is an overlay of `bazel/rules.bzl` from the Perfetto repository.

It is used to fix compatibility issues with modern Bazel by loading `py_library` and `py_binary` from `@rules_python` instead of using the deprecated `native.` functions.
"""
...
def perfetto_cc_protozero_library(name, deps, **kwargs):
    """Generates .pbzero.{cc,h} from .proto(s)."""
    if _rule_override(
...
def perfetto_cc_ipc_library(name, deps, **kwargs):
    """Generates .ipc.{cc,h} and .pb.{cc.h} from .proto(s)."""
...
def perfetto_proto_descriptor(name, deps, outs, **_kwargs):
    args = {
        "name": name,
        "deps": deps,
        "outs": outs,
...
def perfetto_cc_amalgamated_sql(name, deps, outs, namespace, **kwargs):
    """Generates amalgamated SQL headers."""
...
def perfetto_cc_tp_tables(name, srcs, outs, deps = [], **kwargs):
    """Generates trace processor table headers."""
    if PERFETTO_CONFIG.root[:2] != "//":
        fail("Expected PERFETTO_CONFIG.root to start with //")
